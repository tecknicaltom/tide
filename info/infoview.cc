			...
			case K_Return: 
				if (get_xrefs()) {
					...
					if (!x) {
						...
						info_pos *q = (info_pos*)get_xrefs()->enum_prev((Object**)&x, &p);
...
//					fprintf(stderr, "backspace: %s, %s\n", e->file, e->node);
			...
			case K_Tab: {
				if (get_xrefs()) {
					info_pos p(top_line + cursory, xofs + physical_cursorx());
					info_xref *r;
					info_pos *q = (info_pos*)get_xrefs()->enum_next((Object**)&r, &p);
					if (q) {
						goto_line(q->line);
						cursor_pput(q->ofs);
					}
			...
			case K_BackTab: {
				if (get_xrefs()) {
					info_pos p(top_line + cursory, xofs + physical_cursorx());
					info_xref *r;
					info_pos *q = (info_pos*)get_xrefs()->enum_prev((Object**)&r, &p);
					if (q) {
						goto_line(q->line);
						cursor_pput(q->ofs);
					}
