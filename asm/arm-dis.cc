...
int get_arm_regnames(int option, const char **setname, const char **setdescription,
...
static bool print_insn_coprocessor(struct disassemble_info *info, long given,
			bool thumb)
{
					...
					case '5': case '6': case '7': case '8': case '9': {
						...
						case '-':
							...
							case 'f': {
								...
								if (reg > 7) {
									func(stream, "#%s", arm_fp_const[reg & 7]);
								} else {
						...
						case '`':
							c++;
							if ((given & (1 << bitstart)) == 0) func(stream, "%c", *c);
							break;
						case '\'':
							c++;
							if ((given & (1 << bitstart)) != 0) func(stream, "%c", *c);
...
static void print_arm_address(bfd_vma pc, struct disassemble_info *info, long given)
{
	...
	&& ((given & 0x02000000) == 0)) {
		...
		if (given & 0x01000000) {
			if ((given & 0x00800000) == 0) offset = - offset;
			...
			/* Cope with the possibility of write-back
			   ...
			   argue ?  */
			if (given & 0x00200000) func(stream, "!");
		} else {
			/* Post indexed.  */
			func(stream, "], #%d", offset);
		...
		if ((given & 0x01000000) != 0) {
			if ((given & 0x02000000) == 0) {
				int offset = given & 0xfff;
				if (offset) {
					func(stream, ", #%s%d", (((given & 0x00800000) == 0) ? "-" : ""), offset);
				}
			} else {
				func(stream, ", %s", (((given & 0x00800000) == 0) ? "-" : ""));
				arm_decode_shift (given, func, stream);
			}
			func(stream, "]%s", ((given & 0x00200000) != 0) ? "!" : "");
		} else {
			if ((given & 0x02000000) == 0) {
				int offset = given & 0xfff;
				if (offset) {
...
static void print_insn_arm(bfd_vma pc, struct disassemble_info *info, long given)
{
	...
	if (print_insn_coprocessor(info, given, false)) return;
				...
				if (*c == '%') {
						...
						if ((given & 0x004f0000) == 0x004f0000) {
							...
							if ((given & 0x01000000) != 0) {
								...
								if ((given & 0x00400000) == 0x00400000) {
									...
									/* Register.  */
									func(stream, ", %s%s", (((given & 0x00800000) == 0) ? "-" : ""),
								...
								if ((given & 0x00400000) == 0x00400000) {
									...
									/* Register.  */
									func(stream, "], %s%s",(((given & 0x00800000) == 0) ? "-" : ""), arm_regnames[given & 0xf]);
								}
							}
						}
						break;
					...
					case 'o':
						if ((given & 0x02000000) != 0) {
							...
							immed = (((immed << (32 - rotate)) | (immed >> rotate)) & 0xffffffff);
						...
						break;
					case 'p':
						if ((given & 0x0000f000) == 0x0000f000) func(stream, "p");
						break;
					case 't':
						if ((given & 0x01200000) == 0x00200000) func(stream, "t");
						break;
					case 'A':
						func(stream, "[%s", arm_regnames [(given >> 16) & 0xf]);
						if ((given & (1 << 24)) != 0) {
					...
					case 'C':
						func(stream, "_");
						if (given & 0x80000) func(stream, "f");
						if (given & 0x40000) func(stream, "s");
						if (given & 0x20000) func(stream, "x");
						if (given & 0x10000) func(stream, "c");
						break;
					case 'U':
						switch (given & 0xf) {
						...
						default: func(stream, "#%d", (int)given & 0xf);
						}
						break;
					case '0': case '1': case '2': case '3': case '4':
					case '5': case '6': case '7': case '8': case '9': {
...
bool arm_symbol_is_valid (asymbol * sym,
...
static int print_insn(bfd_vma pc, struct disassemble_info *info, bool little)
{
	unsigned char b[4];
	long given;
	int status;
