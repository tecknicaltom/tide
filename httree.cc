...
void UiTreeview::init(Bounds *b, const char *d)
{
	UiView::init(b, VO_SELECTABLE | VO_BROWSABLE/* <- FIXME */ | VO_RESIZE, d);
	VIEW_DEBUG_NAME("UiTreeview");
...
void UiTreeview::done()
{
	UiView::done();
}

void UiTreeview::adjust_focus(int Focus)
{
...
int  UiTreeview::create_graph(AbstractChar *s, void *node, int level, int lines, int width, int endwidth, const AbstractChar *Chars)
{
...
void UiTreeview::collapse_all(void *node)
{
	if (has_children(node)) {
		adjust(node, false);
...
void UiTreeview::draw_r(void *node, int level, int *pos, uint32 lines)
{
...
void UiTreeview::expand_all(void *node)
{
	if (has_children(node)) {
		adjust(node, true);
		void *tmp = get_child(node, 1);
...
/*
 */
void UiTreeview::getdata(ObjectStream &s)
{
	ht_treeview_data d;
	d.selected = selected;
...
/*
 *	void *UiTreeview::get_child(void *node, int i)
 *	get i. child of node
 */

int  UiTreeview::get_graph(AbstractChar *s, void *node, int level, int lines)
{
...
void *UiTreeview::get_node_r(void *node, int *i)
{
	while ((node) && (--(*i))) {
		if (has_children(node) && is_expanded(node)) {
...
void	UiTreeview::handlemsg(htmsg *msg)
{
...
void	UiTreeview::has_focused(int i)
{
	foc = i;
}
 ...
 */
bool	UiTreeview::is_selected(int i)
{
	return (foc == i);
}

void	UiTreeview::scroll_to(int x, int y)
{
	delta_x = x;
	delta_y = y;
}
 ...
 *	stub
 */
void UiTreeview::setdata(ObjectStream &s)
{
}
...
void	UiTreeview::set_limit(int x, int y)
{
	maxsize_x = x;
	maxsize_y = y;
}

void	UiTreeview::update_r(void *node, int level, int *pos, int *x)
{
	AbstractChar s[2048];
...
void UiStaticTreeview::init(Bounds *b, const char *desc)
{
	UiTreeview::init(b, desc);
	VIEW_DEBUG_NAME("UiStaticTreeview");
	root=0;
}
...
void UiStaticTreeview::done()
{
	ht_static_treeviewdone_r(root);
	UiTreeview::done();
}

void *UiStaticTreeview::add_child(void *node, const char *text, Object *Data)
{
	if (node) {
...
void	*UiStaticTreeview::add_node(static_node **node, const char *text, Object *Data)
{
	static_node **p = node;
...
void	UiStaticTreeview::adjust(void *node, bool expand)
{
	((static_node *)node)->expanded = expand;
}

static_node *UiStaticTreeview::create_node(const char *text, static_node *prev, Object *Data)
{
...
void	*UiStaticTreeview::get_prev_node(void *node)
{
	return ((static_node *)node)->prev;
}

void	*UiStaticTreeview::get_root()
{
	return root;
}
...
void	*UiStaticTreeview::get_cursor_node()
{
// FIXME: said to be the wrong thing, works however
	return selected;
}

void	UiStaticTreeview::goto_node(void *node)
{
}
